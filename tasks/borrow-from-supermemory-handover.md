...
- **Carry-forward for a future improvement run:** to actually move the knowledge-updates MRR, add a LongMemEval loader mode that ingests update-pairs through `handlers/remember.handler` (or a thin `try_curation` call) so the detector creates real supersession edges, then measure the per-category delta. Until then Phase 2 is a production-correctness win with a benchmark-neutral footprint.

## 2 — Connection-rooted scoping
...
## 3 — Inline relation-walk recall mode
