...
\citep{Kandel2001,McClelland1995}, and (iv)~WRRF fusion with heat
as a tie-breaker.  On three independent long-term-memory
...
A direct decay dose--response on BEAM isolates the mechanism:
disabling the Ebbinghaus heat decay (setting the per-hour retention
factor to 1.0) drops retrieval-proxy MRR from 0.671 to 0.399
($-41\%$) and R@10 from 0.867 to 0.567, on the same code path.  The
benefit is regime-dependent: it requires a corpus with differential
access history (\S\ref{sec:regime}); on single-pass,
isolated-haystack evaluations decay is neutral-to-slightly-negative,
exactly as its mechanism-of-action predicts.  We characterise when
...
\begin{enumerate}
  ...
  \item A \emph{three-form argument} (\S\ref{sec:why-decay}) that
        ...
        (WRRF tie-breaking), and curation-by-attrition.
...
integration into a six-signal fusion retriever (\S\ref{sec:wrrf}) and
...
mechanism-of-action applies, we run a direct dose--response on
BEAM-100K, varying only the per-hour heat-retention factor
$\lambda_{\mathrm{base}}$ between the engineering default $0.95$ and
$1.0$ (decay fully disabled), holding the entire rest of the stack
and the same \texttt{recall\_memories()} code path fixed.  Artefact:
\texttt{benchmarks/results/decay\_sweep/20260430T111134Z/}
(\texttt{summary.csv}, \texttt{analysis.json}).
...
\midrule
$0.95$ (decay on)  & \textbf{0.671} & \textbf{0.867} \\
$1.00$ (decay off) & 0.399 & 0.567 \\
\midrule
$\Delta$ (on $-$ off) & $+0.272$ ($+68\%$) & $+0.300$ \\
\bottomrule
\end{tabular}
\end{center}

Disabling decay removes $41\%$ of retrieval-proxy MRR
($0.272/0.671$) and $35\%$ of R@10 ($0.300/0.867$) on the same code
path.  The sweep's two-point grid places the optimum at
$\lambda_{\mathrm{base}} = 0.95$ with a measured sensitivity slope
of $\approx -5.44$ MRR per unit $\lambda$ near the decay-off boundary
(\texttt{analysis.json}: \texttt{slope\_right} $= -5.44$); the
plateau width at $\varepsilon = 0.005$ MRR is the single optimum
cell (\texttt{plateau\_width} $= 0$ grid spacing on this two-point
sweep).  This is the dose--response that the isolated-haystack
ablations cannot produce, and it is the load-bearing evidence that
decay is causal in the regime where access history is heterogeneous.
...
The decay dose--response of \S\ref{sec:decay-dose}, run on BEAM
where access history is heterogeneous, measures the mechanism in the
regime where its mechanism-of-action applies, and there decay
contributes $+0.272$ MRR.  The two results are consistent: decay
helps exactly where there is access-history structure to exploit,
and is inert where there is not.
...
\paragraph{Regime controls (preliminary).}
...
structure.  A Zipf-$\alpha{=}1.5$ access pattern at $N=100$ is
likewise saturated (both at MRR 1.0).  The full
$N \in \{10^3,10^4,10^5\}$ curve that would locate the crossover is
in progress; we report the committed points here and defer the curve
to a revision rather than report unreproduced figures.  Independently,
...
detailed writeup at
\texttt{tasks/e1-v3-locomo-results-post-fix.md}.
