...
\usepackage{booktabs}
\usepackage{xcolor}
% \graynoise: marks a per-ability table row whose delta is below the
% single-question noise floor of its slice (see top of Section 5).
\newcommand{\graynoise}[1]{\textcolor{gray}{\textit{#1}}}
...
(2)~a stage-aware assembler (two phases implemented and evaluated;
a third summary-fallback phase is designed but not yet benchmarked) with submodular coverage
selection, Personalized PageRank entity graph traversal, and
schema-structured summary fallback.
On BEAM-10M (196 questions, 10 conversations), the assembler with
ground-truth stage boundaries (BEAM \texttt{plan\_id}) achieves 0.429
MRR, a $+21.5\%$ improvement over the flat baseline (0.353), with 8
of 10 memory abilities improving.  Replacing the oracle labels with
\emph{label-free} temporal stage detection (day-level timestamp
partitioning, no metadata) raises this to 0.471 ($+33.4\%$):
temporal proximity proved a stronger stage signal than topic
boundaries on this benchmark
(\S\ref{sec:temporal-stage-results}), so the architecture deploys
without oracle metadata
(\S\ref{sec:stage-detection-deployment}).
...
We note this is a lower bound on \emph{pairwise-distance} preservation,
not on nearest-neighbor \emph{ordering}, which can be preserved below
the JL bound; we cite it as indicative of the regime, not as a proof
that retrieval must fail at 384 dimensions.  The empirical degradation
in Table~\ref{tab:10m} is the actual evidence.
...
\begin{enumerate}
  \item \textbf{ContextDecomposer}: a priority-budgeted prompt
    ...
    injection---a design contribution we found undescribed in the
    2024--2026 retrieval, prompt engineering, or agent memory
    literature we surveyed, and whose effect on answer quality we do
    not evaluate here.

  \item \textbf{StageAwareContextAssembler}: a hierarchical
    retrieval architecture (two phases evaluated; third designed) with a configurable budget
    split (default 60/30/10, where the 10\% is reserved for the
    designed-but-unwired summary-fallback phase), combining submodular coverage selection
  ...
  \item \textbf{Empirical validation} on BEAM at two scales (100K
    ...
    provides a $+21.5\%$ improvement at large scale with oracle
    stage labels, rising to $+33.4\%$ with label-free temporal stage
    detection (where flat retrieval collapses)---with 8 of 10 memory
    abilities improving in both configurations.
...
\paragraph{Design contribution.}
The truncation warning is, to our knowledge, undescribed in the
2024--2026 literature we surveyed; we present it as a design
contribution and do not evaluate its effect on answer quality here
(an A/B on truncated-evidence questions is the natural test and is
left to future work).  We surveyed the
...
Of the three phases, Phases~1--2 are implemented and evaluated in this
paper; Phase~3 (\S\ref{sec:phase3}) is designed and interface-complete
but returns empty in the evaluated build.
...
\paragraph{Statistical rigor and per-row noise floor.}
The per-ability slices are computed over very few questions: 10
questions per ability at BEAM-100K and 20 per ability at BEAM-10M.
At 100K, a single question therefore moves an ability's MRR by
$\approx 0.1$ (one question $\approx 0.1$ of the slice); at 10M, by
$\approx 0.05$.  Per-ability deltas smaller than this floor
($|\Delta| < 0.1$ at 100K, $|\Delta| < 0.05$ at 10M) are below the
single-question resolution of the slice and should be read as noise;
we render such sub-noise per-ability rows in gray italics.  The
reported $\pm 0.01$ ``variance'' (\S\ref{sec:measurement}) is
run-to-run \emph{determinism} of the pipeline (same DB, restart
between runs), \emph{not} sampling uncertainty over questions: it
quantifies reproducibility, not confidence intervals.  We do not
report question-level bootstrap confidence intervals here; no results
artifact currently contains them, so question-level bootstrap CIs are
...
\midrule
\graynoise{temporal\_reasoning} & \graynoise{0.950} & \graynoise{0.920} & \graynoise{$-0.030$} \\
\graynoise{knowledge\_update} & \graynoise{0.850} & \graynoise{0.900} & \graynoise{$+0.050$} \\
\graynoise{contradiction\_resolution} & \graynoise{0.817} & \graynoise{0.783} & \graynoise{$-0.034$} \\
multi\_session\_reasoning & 0.812 & 0.500 & $\mathbf{-0.312}$ \\
event\_ordering & 0.380 & 0.525 & $\mathbf{+0.145}$ \\
info\_extraction & 0.543 & 0.700 & $\mathbf{+0.157}$ \\
\graynoise{instruction\_following} & \graynoise{0.448} & \graynoise{0.478} & \graynoise{$+0.030$} \\
\graynoise{preference\_following} & \graynoise{0.442} & \graynoise{0.442} & \graynoise{$0.000$} \\
\graynoise{abstention} & \graynoise{0.400} & \graynoise{0.400} & \graynoise{$0.000$} \\
\graynoise{summarization} & \graynoise{0.271} & \graynoise{0.370} & \graynoise{$+0.099$} \\
\midrule
...
\begin{table}[t]
\centering
\caption{BEAM-10M results (10 conversations, 196 questions; assembler
  with \emph{oracle} \texttt{plan\_id} stage boundaries; artefact
  \texttt{benchmarks/beam/variance/assembler\_10m\_stagefixed.txt}).
  $\Delta$ = Assembler $-$ WRRF.  LIGHT scores are end-to-end QA
  (LLM-as-judge, Llama-4-Maverick); not comparable to retrieval MRR.
  Shown for directional reference only.  Label-free temporal stage
  detection scores higher still (\S\ref{sec:temporal-stage-results}).}
\label{tab:10m}
\small
...
\graynoise{info\_extraction} & \graynoise{0.448} & \graynoise{0.487} & \graynoise{$+0.039$} & \graynoise{70\%} & \graynoise{70\%} & \graynoise{---} \\
...
\graynoise{event\_ordering} & \graynoise{0.067} & \graynoise{0.067} & \graynoise{$0.000$} & \graynoise{10\%} & \graynoise{10\%} & \graynoise{0.266} \\
\graynoise{summarization} & \graynoise{0.186} & \graynoise{0.150} & \graynoise{$-0.036$} & \graynoise{22\%} & \graynoise{22\%} & \graynoise{0.277} \\
\midrule
...
The only regression, and the predicted cost of leaving Phase~3
unwired.  Summarization questions require broad coverage
across many stages.  Stage-scoped retrieval focuses depth within a
stage at the cost of breadth---exactly the breadth that the designed
Phase~3 summary fallback is intended to restore.  Phase~3 is a stub
in the evaluated build, so this regression is the expected consequence
of the missing phase rather than a property of the architecture as
designed.

\subsection{Scale-Dependent Behavior (Conjecture)}
...
Empirically, the embedding space transitions from underpopulated
(94 memories) to overcrowded (7,500 memories) between the two scales;
the hubness, concentration, and JL effects of \S\ref{sec:scaling-failure}
are all monotone in corpus size at fixed dimension, so we expect---and
observe---degradation that worsens with $N$.

The crossover point---where structured assembly's benefit exceeds its
overhead---lies between these two measured scales, corresponding to
roughly 400--7,500 memories in 384 dimensions, or approximately 400K
to 10M tokens of conversation.  We did not evaluate at intermediate
scales; we expect the crossover to lie in this range and the benefit
to increase with corpus size, but this is a conjecture, not a measured
result.
% TODO (compute-dependent, separate run): the BEAM 500K and 1M splits
% exist and can be benchmarked to locate the crossover empirically and
% replace this conjecture with a measured value.
...
gated by corpus density}.  At small corpus sizes (underpopulated
embedding space), use flat retrieval; at large corpus sizes
(overcrowded embedding space), enable the assembler.
...
\subsection{Label-Free Temporal Stage Detection Outperforms the Oracle}
\label{sec:temporal-stage-results}

The Table~\ref{tab:10m} results use BEAM's ground-truth
\texttt{plan\_id} stage boundaries---metadata a production system
does not have.  To measure the deployable configuration, we re-ran
the identical BEAM-10M protocol (same 10 conversations, same 196
questions, same code revision) with the
\texttt{TemporalStageDetector}: stages are induced purely from
memory timestamps by day-level gap partitioning, with no access to
\texttt{plan\_id} or any other label.  Artefact:
\texttt{benchmarks/beam/variance/assembler\_10m\_temporal.txt}.

\begin{table}[t]
\centering
\caption{BEAM-10M, label-free temporal stage detection (same
  conversations, questions, and code revision as
  Table~\ref{tab:10m}).  $\Delta$ = temporal assembler $-$ WRRF
  baseline.}
\label{tab:10m-temporal}
\small
\begin{tabular}{lccc}
\toprule
Ability & MRR & R@10 & $\Delta$ vs.\ WRRF \\
\midrule
knowledge\_update & \textbf{0.950} & 100.0\% & $+0.115$ \\
contradiction\_res. & \textbf{0.892} & 95.0\% & $+0.259$ \\
info\_extraction & \textbf{0.592} & 75.0\% & $+0.144$ \\
abstention & \textbf{0.600} & 60.0\% & $+0.500$ \\
preference\_follow. & \textbf{0.508} & 60.0\% & $+0.096$ \\
temporal\_reasoning & \textbf{0.460} & 50.0\% & $+0.090$ \\
multi\_session\_reas. & 0.425 & 60.0\% & $+0.010$ \\
instruction\_follow. & 0.150 & 15.0\% & $+0.082$ \\
summarization & 0.083 & 11.1\% & $-0.103$ \\
event\_ordering & 0.050 & 5.0\% & $-0.017$ \\
\midrule
\textbf{Overall MRR} & \textbf{0.471} & 53.1\% & $+0.118$ (\textbf{+33.4\%}) \\
\bottomrule
\end{tabular}
\end{table}

The label-free configuration scores \textbf{0.471 overall---higher
than the 0.429 oracle configuration} ($+0.042$).  This was not
predicted.  Day-level temporal partitioning groups same-day memories
into one stage, and on conversational memory this proves a stronger
stage signal than BEAM's topic boundaries: knowledge updates and
contradictions cluster in time (the newer fact arrives in a later
stage), and temporal scoping empties irrelevant stages, driving the
largest single gain (abstention $+0.500$).  Two abilities regress
relative to WRRF: summarization ($-0.103$, steeper than the oracle
configuration's $-0.036$; same structural cause,
\S\ref{sec:summarization-tradeoff}) and event ordering ($-0.017$,
unchanged under oracle labels)---chronological sequencing needs more
than retrieval in either configuration.  We emphasise the practical
consequence rather than a general claim: \emph{on this benchmark}
the assembler does not need oracle metadata; whether temporal
partitioning transfers to corpora whose topic structure is
decoupled from time is an open question.

\paragraph{Reproduction on a later code revision.}
Both configurations were re-run two months later on the
then-current retrieval stack (which had absorbed unrelated scoring
revisions), each against a freshly created database, same 196
questions: oracle 0.496 MRR / R@10 62.5\%, temporal \textbf{0.523}
MRR / R@10 59.3\% (artefact
\texttt{benchmarks/results/beam10m\_paired/RESULTS.md}).  The
temporal MRR advantage reproduces ($+0.027$, vs.\ $+0.042$ on the
original revision), and the same fine structure recurs in both
pairs: temporal wins MRR while oracle holds a small R@10 edge---
temporal partitioning ranks the gold memory higher when it is
retrieved, while oracle boundaries retrieve it slightly more often.
Absolute numbers are comparable only within a pair, not across code
revisions.

\subsection{Comparison with LIGHT}
\label{sec:light-comparison}

LIGHT achieves 0.266 overall on BEAM-10M.  We achieve 0.471 on
retrieval-proxy MRR with the label-free temporal configuration
(0.429 with oracle labels).  \textbf{These numbers are not directly
comparable.}
...
lower the score even when retrieval is correct.  Our 0.471 is
...
our 0.471.  This is speculative arithmetic, not a rigorous comparison.
...
Summarization regresses at 10M scale ($-0.036$).  This is the
predicted cost of Phase~3 (summary fallback) being unwired: stage-scoped
retrieval focuses depth at the cost of breadth, and the designed Phase~3
is the component intended to restore that breadth.  It is currently a
stub.  Wiring it to Cortex's CLS consolidation engine is the immediate
...
\label{sec:stage-detection-deployment}

Production deployment requires stage detection without explicit
labels.  The TemporalStageDetector is now benchmarked on BEAM-10M
and outperforms the oracle labels there
(\S\ref{sec:temporal-stage-results}), so label-free deployment is
demonstrated for conversational memory whose topic structure
correlates with time.  The open question is narrower than before:
corpora where topic structure is decoupled from time (interleaved
projects, bursty multi-topic sessions) may need content-based stage
detection, which remains unbenchmarked.
...
manages retrieval-level composition through two implemented phases
(with a third designed): own-stage
retrieval with submodular diversity \citep{Krause2008}, cross-stage
bridging via Personalized PageRank over the entity graph
\citep{Gutierrez2024}, and a designed summary-fallback phase for
uncovered stages \citep{Tse2007}.
...
and provides a $+21.5\%$ improvement at 10M tokens with oracle stage
labels---rising to $+33.4\%$ label-free
(\S\ref{sec:temporal-stage-results})---with 8 of 10 memory abilities
improving in both configurations.  The multi-session reasoning sign
flip (from $-0.312$ at 100K to $+0.128$ at 10M, oracle
configuration) is the strongest evidence that the benefit is
genuinely scale-dependent rather than incidental.
...
After stage fix (oracle labels) & 0.429 & Correct stage assignment \\
Temporal detector replaces oracle & 0.471 & Label-free (\S\ref{sec:temporal-stage-results}) \\
\bottomrule
\end{tabular}
\end{table}
