...
This result uses ground-truth stage boundaries (BEAM \texttt{plan\_id});
we report it as an upper bound on the assembler's benefit and discuss
label-free stage detection, which we have not yet benchmarked, as the
primary open question for deployment (\S\ref{sec:stage-detection-deployment}).
...
\begin{enumerate}
  ...
  \item \textbf{Empirical validation} on BEAM at two scales (100K
    ...
    provides a $+33.4\%$ improvement at large scale (where flat
...
\midrule
\textbf{Overall MRR} & \textbf{0.353} & \textbf{0.471} & $\mathbf{+0.076}$ \textbf{(+33.4\%)} & & & 0.266$^*$ \\
\bottomrule
\end{tabular}
\end{table}
...
and provides a $+33.4\%$ improvement at 10M tokens, with 8 of 10
...
After stage fix (final) & 0.471 & Correct stage assignment \\
\bottomrule
\end{tabular}
\end{table}

The jump from 0.120 to 0.471 upon fixing the stage detection confirms
